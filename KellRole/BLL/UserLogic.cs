...
namespace KellRole
{
    public class UserLogic
    {
        ...
        {
            string sql = "update [User] set Username='" + user.Username + "',Password='" + user.Password + "', Depart='" + Common.GetDepartmentsStr(user.Departments) + "', Flag=" + user.Flag + ", Remark='" + user.Remark + "' where ID=" + user.ID;
        ...
        public bool UpgradeList(List<User> list)
        {
            ...
            {
                string sqlStr = "if exists (select 1 from [User] where ID=" + user.ID + ") update [User] set Username='" + user.Username + "',Password='" + user.Password + "', Depart='" + Common.GetDepartmentsStr(user.Departments) + "', Flag=" + user.Flag + ", Roles='" + Common.GetRolesStr(user.Roles) + "', Usergroup='" + Common.GetUserGroupsStr(user.Usergroups) + "', Remark='" + user.Remark + "' where ID=" + user.ID + " else insert into [User] (Username, Password, Depart, Flag, Roles, Usergroup, Remark) values ('" + user.Username + "','" + user.Password + "','" + Common.GetDepartmentsStr(user.Departments) + "'," + user.Flag + ",'" + Common.GetRolesStr(user.Roles) + "','" + Common.GetUserGroupsStr(user.Usergroups) + "','" + user.Remark + "')";
                try
                {
        ...
        {
            return sqlHelper.Exists("select 1 from [User] where Username='" + name + "'");
        }
        ...
        {
            return sqlHelper.Exists("select 1 from [User] where ID!=" + myId + " and Username='" + name + "'");
        }
        ...
        public bool ExistsWhere(string where)
        {
            if (!string.IsNullOrEmpty(where))
            {
                ...
                if (!w.StartsWith("where "))
                    w = "where " + w;
                return sqlHelper.Exists("select 1 from [User] " + w);
            }
            return false;
        }
    }
