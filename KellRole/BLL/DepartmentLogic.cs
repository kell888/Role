...
namespace KellRole
{
    public class DepartmentLogic
    {
        ...
        {
            string sql = "select Parent from Depart where ID=" + id;
            DataTable dt = sqlHelper.Query(sql);
            if (dt != null && dt.Rows.Count > 0)
            {
        ...
        public bool UpdateDepartment(Department dep)
        {
            ...
            //if (dep.Parent != null)
            //    parent = dep.Parent.ID.ToString();
            string sql = "update Depart set Name='" + dep.Name + "', Manager='" + dep.Manager + "', Parent=" + dep.ParentID + ", Remark='" + dep.Remark + "' where ID=" + dep.ID;
        ...
        {
            string sql = "delete from Depart where ID=" + dep.ID;
            int r = sqlHelper.ExecuteSql(sql);
            return r > 0;
        }
        /// <summary>
        ...
        public bool UpgradeList(List<Department> list)
        {
            ...
            foreach (Department dep in list)
            {
                ...
                //if (dep.Parent != null)
                //    parent = dep.Parent.ID.ToString();
                string sqlStr = "if exists (select 1 from Depart where ID=" + dep.ID + ") update Depart set Name='" + dep.Name + "', Manager='" + dep.Manager + "', Parent=" + dep.ParentID + ", Roles='" + Common.GetRolesStr(dep.Roles) + "', Remark='" + dep.Remark + "' where ID=" + dep.ID + " else insert into Depart (Name, Manager, Parent, Roles, Remark) values ('" + dep.Name + "','" + dep.Manager + "', " + dep.ParentID + ", '" + Common.GetRolesStr(dep.Roles) + "', '" + dep.Remark + "')";
                try
                {
        ...
        {
            return sqlHelper.Exists("select 1 from Depart where Name='" + name + "'");
        }
        ...
        {
            return sqlHelper.Exists("select 1 from Depart where ID!=" + myId + " and Name='" + name + "'");
        }
        ...
        public bool ExistsWhere(string where)
        {
            if (!string.IsNullOrEmpty(where))
            {
                ...
                if (!w.StartsWith("where "))
                    w = "where " + w;
                return sqlHelper.Exists("select 1 from Depart " + w);
            }
            return false;
        }
    }
