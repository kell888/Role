...
namespace KellRole
{
    public class RoleLogic
    {
        ...
        {
            string sql = "update [Role] set Name='" + role.Name + "', Permissions='" + Common.GetPermissionsStr(role.Permissions) + "', Flag=" + (role.Flag ? "1" : "0") + ", Remark='" + role.Remark + "' where ID=" + role.ID;
        ...
        {
            string sql = "delete from [Role] where ID=" + role.ID;
            int r = sqlHelper.ExecuteSql(sql);
            return r > 0;
        }
        /// <summary>
        ...
        public bool UpgradeList(List<Role> list)
        {
            ...
            {
                string sqlStr = "if exists (select 1 from [Role] where ID=" + role.ID + ") update [Role] set Name='" + role.Name + "', Permissions='" + Common.GetPermissionsStr(role.Permissions) + "', Flag=" + (role.Flag ? "1" : "0") + ", Remark='" + role.Remark + "' where ID=" + role.ID + " else insert into [Role] (Name, Permissions, Flag, Remark) values ('" + role.Name + "', '" + Common.GetPermissionsStr(role.Permissions) + "', " + (role.Flag ? "1" : "0") + ", '" + role.Remark + "')";
                try
                {
        ...
        {
            return sqlHelper.Exists("select 1 from [Role] where Name='" + name + "'");
        }
        ...
        {
            return sqlHelper.Exists("select 1 from [Role] where ID!=" + myId + " and Name='" + name + "'");
        }
        ...
        public bool ExistsWhere(string where)
        {
            if (!string.IsNullOrEmpty(where))
            {
                ...
                if (!w.StartsWith("where "))
                    w = "where " + w;
                return sqlHelper.Exists("select 1 from [Role] " + w);
            }
            return false;
        }
    }
